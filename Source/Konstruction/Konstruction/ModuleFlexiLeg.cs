...
namespace Konstruction
{
    ...
    {
        [KSPField] public string menuName = "Angle";

        //A series of transforms and their increments
        [KSPField] public string transformConfig;

        [KSPField(guiName = "Angle", isPersistant = true, guiActive = true, guiActiveEditor = true),
         UI_FloatRange(stepIncrement = 1f, maxValue = 100f, minValue = 0f)] public float servoPercent = 0f;

        [KSPField] public float servoSpeed = 0.1f;

        [KSPField] public float minValue = 0f;

        [KSPField] public float maxValue = 150f;

        [KSPField] public string wheel_standin = "WheelPivot_Standin";

        [KSPField] public string suspension_standin = "suspensionPivot_Standin";

        [KSPField] public string steering_standin = "SteeringPivot_Standin";

        public float currentValue = 0f;

        [KSPField] public string wheelTransformName = "WheelPivot";

        [KSPField] public string suspensionTransformName = "suspensionPivot";

        [KSPField] public string steeringTransformName = "SteeringPivot";
        ...
        public override void OnStart(StartState state)
        {
            ...
            {
                transformKeys.Add(tList[i],
                    new Vector3(float.Parse(tList[i + 1]), float.Parse(tList[i + 2]), float.Parse(tList[i + 3])));
            }
            currentValue = 0;
            CheckRotations(1000);
        }

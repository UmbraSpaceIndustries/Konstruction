...
namespace Konstruction.Utilities
{
    ...
    {
        public static List<CostData> GetPartCost(AvailablePart part)
        {
            var costs = new List<CostData>();
            double runningCost = 0;
            double runningMass = 0;

            var baseMult = KonstructionScenario.Instance.settings.GetSettings().massMultiplier;
            var baseResourceName = KonstructionScenario.Instance.settings.GetSettings().massResourceName;
            var costResourceName = KonstructionScenario.Instance.settings.GetSettings().costResourceName;
            ...
            CostData baseCost = new CostData();
            baseCost.Quantity = (int)Math.Ceiling(part.partPrefab.mass * baseMult / baseResource.density);
            baseCost.Resource = baseResource;
            costs.Add(baseCost);
            ...
            foreach (var module in KonstructionScenario.Instance.settings.GetModuleResources())
            {
                if (part.partPrefab.Modules.Contains(module.moduleName))
                {
                    ...
                    modCost.Resource = moduleResource;
                    modCost.Quantity = (int)Math.Ceiling(part.partPrefab.mass * module.massMultiplier / moduleResource.density);
                    costs.Add(modCost);
            ...
            foreach (var costRes in KonstructionScenario.Instance.settings.GetCostResources())
            {

    ...
    stage("Build") {
      steps {
        powershell '''
          ...
          dotnet build --output FOR_RELEASE/GameData/UmbraSpaceIndustries/Konstruction --configuration $env:BUILD_CONFIG `
            --verbosity detailed /p:ReferencePath="$ReferencePath" ./Source/KonstructionUI/KonstructionUI.csproj
          dotnet build --output FOR_RELEASE/GameData/UmbraSpaceIndustries/Konstruction --configuration $env:BUILD_CONFIG `
            --verbosity detailed /p:ReferencePath="$ReferencePath" ./Source/Konstruction/Konstruction.csproj
        '''
      }
    }
    ...
    stage("Package artifacts") {
      steps {
        powershell '''
          ...
          Copy-Item -Path ./*.txt -Destination ./artifacts/GameData/UmbraSpaceIndustries/Konstruction
